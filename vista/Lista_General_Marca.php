...
<head lang="es">
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1">
		...
		<script>
 		  ...
 		  <?php foreach (lista_marca() as $ListC) { ?> //PASAMOS METODO LISTA DE CATEGORIA
 		  	incrementador += 1;
 		  	var Prueba_php = [ <?php echo $ListC['id_marca']; ?> ]; //VALORES QUE LE VAMOS A PASAR AL ARREGLO
						    ...
						    primary: {
						      ...
						      callback: function() {
						      	 ...
						      	 $.ajax({
										async: false,
										type: 'POST',
										url: 'Capturar_Eliminar_Marca.php',
										data: datos, 
										contentType:false,
										processData:false,
...
<div class="panel panel-default">
  <!-- Default panel contents -->
  	<div class="panel-heading"> <i class="fa fa-list-alt"></i>  <strong> LISTA DE MARCAS</strong> </div>
...
<?php foreach (lista_marca() as $ListC) {?>

	<tr>
	<td><?php echo $ListC['nombre_marca']; ?></td>

	<td> <input type="button" id="<?php echo 'eliminar-'.$ListC["id_marca"]; ?>" class="btn btn-danger" value="Eliminar" /> <input type="button" id="<?php echo 'editar-'.$ListC["id_marca"]; ?>" class="btn btn-default" value="Editar" /> </td>
<?php }  ?>
   </tr>
  </table> 
 </div>	
		 ...
		 <hr>
		 	...
		 	<form id="formu_editar">
			 	...
			 	<div class="form-group" hidden>
							    ...
							    <input type="text" class="form-control" id="txt-id_marca" name="id_marca" >
				</div>

				<div class="form-group">
							    <label for="Nombre">Nombre de Marca</label>
							    <input type="text" class="form-control" id="txt-nombre" name="nombre_marca" >
				</div>
